...
```{r ex_create_string}
# Your code
string_single <- "It's Thursday"
string_double <- 'It\'s Thursday'
...
Using double quotes is easier for this string because the apostrophe in `It's` does not need to be escaped. If I use single quotes, I need to escape the apostrophe with `\'`.
...
path_string <- "C:\\Users"
path_string
str_view(path_string)
```

If I do not escape the backslash, R treats `\` as the start of a special character sequence, so the string will not be interpreted correctly.
...
df |>
    mutate(
        middle_pos = ceiling(str_length(word) / 2),
        middle_letter = str_sub(word, start = middle_pos, end = middle_pos)
    )
...
# Count
posts |>
    summarize(num_love = sum(str_count(question_only, "love")))

# Detect
posts |>
    mutate(has_love = str_detect(question_only, "love")) |>
    count(has_love)

# Subset
str_subset(posts$question_only, "love") |> head()

# Extract
str_extract(posts$question_only, "love") |> head()

# Extract all
str_extract_all(posts$question_only, "love") |> head()

# Replace
str_replace(posts$question_only[1:5], "love", "care")

# Replace all
str_replace_all(posts$question_only[1:5], "love", "care")

# Remove
str_remove(posts$question_only[1:5], "love")

# Remove all
str_remove_all(posts$question_only[1:5], "love")
```
