...
### Layout
The app has a sidebar on the left with user controls and explanatory text. The main area on the right contains multiple linked visualizations, including a scatterplot, a map, and a bar chart. The screenshots also show different sections or tabs for exploring metros, comparing over time, and reading about the project.

### Interactivity features
The app includes a dropdown for selecting a metropolitan area and interactive plots that respond to the chosen city. Users can hover, zoom, and pan to inspect values in more detail. The map and charts are linked, so the app supports deeper exploration than a static set of plots.

### Is the interactivity needed?
Yes. The interactivity is useful because the app is meant to help users explore many cities and neighborhoods more quickly and deeply than static graphics would allow. The linked views make it easier to examine patterns from more than one angle at once.

### What comparisons or explorations are not enabled well?
The app does not make side-by-side comparison between two cities especially easy. It also could do more to summarize the key takeaway after the user explores. Some parts of the layout feel crowded, so a cleaner structure or stronger visual hierarchy would help.
...
data_by_dist <- readRDS("neighborhood_diversity/data/diverse_data_by_dist.rds")
data_by_year <- read_csv("neighborhood_diversity/data/diverse_data_by_year.csv")
...
server <- function(input, output) {
    output$scatter_plot <- renderPlot({
      data_by_dist |>
        ...
        geom_smooth(se = FALSE, span = input$span_value) +
        labs(
          x = "Distance to city hall (miles)",
          y = "Diversity score",
          title = "Diversity score vs. distance to city hall"
          ) +
        theme_minimal()
      })
}
