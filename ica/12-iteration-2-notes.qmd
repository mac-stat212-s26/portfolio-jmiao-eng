...
```{r}
# Step 1: create a storage container to hold one class per column
col_classes <- vector("character", length = ncol(mtcars))

# Step 2: loop over the columns of mtcars
for (i in seq_along(mtcars)) {
  col_classes[i] <- class(mtcars[[i]])
}

# Step 3: add column names so the output is easier to read
names(col_classes) <- colnames(mtcars)

col_classes
```
First, I created a character vector to store the output. Then I looped through each column of `mtcars`, used `class()` on that column, and saved the result in the storage container. Finally, I added the column names to make the output easier to interpret.
...
```{r purrr_col_classes, error=TRUE}
...
```{r class_diamonds_cut, error=TRUE}
class(diamonds$cut)
```
...
```{r purrr_col_classes_no_error, error=TRUE}
map(diamonds, class)
```
...
```{r purrr_examples_by_col, error=TRUE}
...
diamonds_stats_map <- map(diamonds_num, summ_stats) |>
  bind_rows() |>
  mutate(variable = colnames(diamonds_num), .before = 1)

diamonds_stats_map
```

```{r}
diamonds_stats_loop <- vector("list", length = ncol(diamonds_num))

for (i in seq_along(diamonds_num)) {
  diamonds_stats_loop[[i]] <- summ_stats(diamonds_num[[i]])
}

diamonds_stats_loop <- bind_rows(diamonds_stats_loop) |>
  mutate(variable = colnames(diamonds_num), .before = 1)

diamonds_stats_loop
```

I prefer the `map()` approach because it is shorter and easier to read. It makes the repeated task more obvious and avoids manually creating and updating a storage object. The `for` loop is still useful for understanding the logic, but the `map()` version feels cleaner.
...
```{r}
# Example 1: varying string, start, and end
sub_args1 <- tibble(
  string = c("Macalester", "DataScience", "Iteration"),
  start = c(1, 5, 2),
  end = c(4, 8, 6)
)

pmap_chr(sub_args1, str_sub)

# Example 2: fixed string, varying start and end
sub_args2 <- tibble(
  start = c(1, 3, 6),
  end = c(2, 5, 9)
)

pmap_chr(sub_args2, str_sub, string = "Macalester")
```
...
model_results <- df_arg_combos |>
  mutate(model = pmap(
    list(mod_formula = mod_formula, remove_outliers = remove_outliers),
    ~ fit_model(data = diamonds, mod_formula = ..1, remove_outliers = ..2)
  ))
