...
```{r}
diamonds |>
  mutate(across(.cols = c(x, y, z), .fns = ~ str_c(.x, " mm")))
```
...
```{r}
df_arg_combos <- crossing(mod_formula = c(price ~ carat, price ~ carat + cut,  price ~ carat + cut + clarity,  price ~ carat + cut + clarity + color), remove_outliers = c(TRUE, FALSE))
df_arg_combos
```
...
```{r eval = FALSE}
model_storage <- vector("list", length = nrow(df_arg_combos))

for (i in seq_len(nrow(df_arg_combos))) {
  this_formula <- df_arg_combos$mod_formula[[i]]
  this_remove <- df_arg_combos$remove_outliers[i]
  
  model_storage[[i]] <- fit_model(
    data = diamonds,
    mod_formula = this_formula,
    remove_outliers_flag = this_remove
  )
}
